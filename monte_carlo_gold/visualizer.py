...
def plot_historical_prices(data, save_path="visualizations"):
    """Plots historical gold prices with a trend line."""
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    plt.figure(figsize=(12, 6))
    plt.plot(data.index, data['Gold_INR_Gram'], label='Gold Price (INR/Gram)')
    
    # Trend line
    z = np.polyfit(range(len(data)), data['Gold_INR_Gram'], 1)
    p = np.poly1d(z)
    plt.plot(data.index, p(range(len(data))), "r--", label="Trend Line")
    
    plt.title('Historical Gold Prices in INR (2015-2025)')
    plt.xlabel('Date')
...
def plot_simulation_paths(simulations, analysis_results, save_path="visualizations"):
    ...
    plt.title('Monte Carlo Simulation of Gold Prices (100 Sample Paths)')
    plt.xlabel('Days')
    plt.ylabel('Price (INR per Gram)')
    plt.legend()
    plt.grid(True)
