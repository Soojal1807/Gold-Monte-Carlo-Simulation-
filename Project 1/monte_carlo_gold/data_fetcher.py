...
def fetch_gold_data(ticker="GC=F", start_date="2015-01-01", end_date=None):
    """
    Fetches historical gold price data from Yahoo Finance.

    Args:
        ticker (str): The ticker symbol for gold (default: "GC=F" for Gold futures).
        start_date (str): The start date for the data in "YYYY-MM-DD" format.
        end_date (str): The end date for the data in "YYYY-MM-DD" format. If None, fetches up to the current date.

    Returns:
        pd.DataFrame: A DataFrame containing the historical gold price data, or None if fetching fails.
    """
    if end_date is None:
