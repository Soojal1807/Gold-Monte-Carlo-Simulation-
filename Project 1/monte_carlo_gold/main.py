...
from data_fetcher import fetch_gold_data, fetch_inr_data
...
def generate_executive_summary(historical_stats, analysis):
    ...
    summary = f"""
# Executive Summary: Monte Carlo Simulation of Gold Prices in INR
...
- **Insight:** Based on the historical trend and simulation, [Your interpretation here, e.g., "gold shows a positive expected return, but investors should be aware of the significant downside risk as indicated by the VaR and the wide confidence interval."]
...
def main():
    ...
    print("Starting Monte Carlo Simulation for Gold Price...\n")
    create_output_directories()

    # 1. Data Collection
    print("Step 1: Fetching historical data...")
    gold_data = fetch_gold_data(GOLD_TICKER, START_DATE, END_DATE)
    inr_data = fetch_inr_data(INR_TICKER, START_DATE, END_DATE)
    if gold_data is None or inr_data is None:
    ...
    processed_data, historical_stats = process_data(gold_data, inr_data)
    if processed_data is None:
        print("Failed to process data. Exiting.")
        return
    print("Data processed successfully.")
    print(f"  - Latest Gold Price: {historical_stats['latest_price']:.2f} INR/Gram")
